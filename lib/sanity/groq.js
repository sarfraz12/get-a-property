...
*[_type == "post" && slug.current == $slug][0] {
  ...
  "noIndex": noIndex,
  // "gallery": ahora puede traer fotos (type "image", igual que
  // siempre) O videos (type "galleryVideo", ver post.js). "..." sólo
  // trae el archivo de video como una referencia cruda al asset (no
  // sirve para reproducirlo) -- para los ítems de video se resuelve
  // acá mismo la URL directa (mismo patrón que "videoUrl" en
  // hero.slides, lib/sanity/schemas/landingPage.js), así el frontend
  // usa item.videoUrl tal cual sin tener que procesar ningún _ref.
  "gallery": gallery[]{
    ...,
    _type == "galleryVideo" => {
...
*[_type == "landingPage"] {
  ...
  "contactCtaTitle": contactCtaTitle[$lang],
  "contactCtaDescription": contactCtaDescription[$lang],
  "contactCtaButtonText": contactCtaButtonText[$lang],
...
// NUEVO: documentos "legalPage" con su propia página en
// /{lang}/legal/{slug} (ver app/(website)/[lang]/legal/[slug]/page.js).
// Mismo patrón que singlequery/pathquery para "post" arriba.
// ============================================================

// Página legal por slug (para la página en sí y su generateMetadata).
...
// Slugs para generateStaticParams (mismo patrón que "pathquery").
export const legalpagepathquery = groq`
*[_type == "legalPage" && defined(slug.current)][].slug.current
`;

// Listado liviano (título + slug) para el pie de página -- ver
// components/navigation/footer.js. Sólo trae las que el editor eligió
// mostrar ahí ("showInFooter" != false, así que si el campo no existe
// todavía en un documento viejo igual se muestra por defecto).
export const alllegalpagesquery = groq`
*[_type == "legalPage" && showInFooter != false && defined(slug.current)] | order(order asc, title.en asc) {
  "title": title[$lang],
  "slug": slug.current,
}
`;
