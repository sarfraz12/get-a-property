...
export default {
  ...
  fieldsets: [
    ...
    { name: 'contactCtaSection', title: 'Seccion "Tarjeta de contacto" (CTA final, junto al formulario)', options: { collapsible: true, collapsed: true } },
  ...
  fields: [
    ...
    // ================================================================
    // Sección "¿Todavía tienes una pregunta?" (tarjeta CTA negra final,
    // components/sections/ContactCtaSection.tsx)
    // ================================================================
    // BUG REAL encontrado y corregido: home.js renderizaba
    // <ContactCtaSection lang={lang} post={posts?.[0]} /> SIN pasarle
    // title/description/buttonText -- el componente ya sabía recibir
    // esos props (con "??" cae al copy de respaldo si vienen vacíos),
    // pero como nunca se los pasaba, la tarjeta negra ("¿Todavía tienes
    // una pregunta?", junto al teaser del último post) SIEMPRE mostraba
    // el copy fijo del componente, sin ninguna forma de editarlo desde
    // Sanity. Estos 3 campos nuevos se conectan en home.js -- si se
    // dejan vacíos, sigue aplicando el mismo copy de respaldo bilingüe
    // de siempre (nunca queda en blanco).
    {
      name: "contactCtaTitle",
      title: "Título",
      description: 'Ejemplo (ES): "¿Todavía tienes una pregunta?". Example (EN): "Do you still have a question?".',
      type: 'object',
      fieldset: 'contactCtaSection',
      fieldsets: [{ name: 'translations', title: 'Translations', options: { collapsible: true } }],
      fields: supportedLanguages.map(lang => ({
        title: lang.title,
        name: lang.id,
    ...
    },
    {
      ...
      title: "Texto del Botón (formulario)",
      description: 'Ejemplo (ES): "Contáctanos". Example (EN): "Contact us".',
      type: 'object',
      fieldset: 'contactCtaSection',
      fieldsets: [{ name: 'translations', title: 'Translations', options: { collapsible: true } }],
      fields: supportedLanguages.map(lang => ({
    ...
    },
    {
      ...
      // https://www.getaproperty.com.pa/{lang} en sharedMetaData); se deja
